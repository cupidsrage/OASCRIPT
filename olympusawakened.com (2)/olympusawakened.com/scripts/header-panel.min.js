...
(function initHeaderPanel() {
  ...
  const markerConfigs = {
    gaia: { offsetLeft: 0, offsetTop: 0, width: 100, height: 100, useTileCenter: true },
    aerion: { offsetLeft: 9, offsetTop: 9, width: 82, height: 82, useTileCenter: true },
  };
  ...
  if (!hasChatPanel) {
    ...
    function pollHudState() {
      fetch(HUD_STATE_URL, { credentials: 'same-origin' })
        .then((res) => {
          if (!res.ok) return null;
          return res.json();
        })
