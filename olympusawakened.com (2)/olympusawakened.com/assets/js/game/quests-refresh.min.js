'use strict';
(function initQuestRefresh() {
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', setupQuestRefresh)
    } else {
        setupQuestRefresh()
    }
})();

function setupQuestRefresh() {
    window.QuestRefresh = window.QuestRefresh || {};
    window.QuestRefresh.refreshAvailable = refreshAvailableQuests;
    window.QuestRefresh.refreshActive = refreshActiveQuests
}
...
function refreshAvailableQuests() {
    ...
    fetch('api/quests_available.php', {
        method: 'POST',
        headers: {
        ...
        renderAvailableQuestsPanel(panel, data && data.quests ? data.quests : [])
    }).catch(() => {})
}

function refreshActiveQuests() {
    const panel = document.querySelector('[data-quests-active-panel]');
    if (!panel) {
        return
    }
    fetch('api/quests_active.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
        },
        ...
        if (!response.ok) {
            throw new Error('Failed to refresh active quests')
        }
        return response.json()
    }).then((data) => {
        renderActiveQuestsPanel(panel, data && data.quests ? data.quests : [])
    }).catch(() => {})
}

function renderAvailableQuestsPanel(panel, quests) {
    panel.innerHTML = '';
    ...
    quests.forEach((quest) => {
        ...
        let iconPath = (window.questItemPlaceholderSrc && String(window.questItemPlaceholderSrc)) || 'assets/img/ui/quest_item_placeholder.jpg';
        if (rawIcon) {
...
function renderActiveQuestsPanel(panel, quests) {
    panel.innerHTML = '';
    if (!Array.isArray(quests) || quests.length === 0) {
        const emptyEl = document.createElement('div');
        emptyEl.className = 'text-gray-400';
        emptyEl.textContent = 'You have no active quests.';
        panel.appendChild(emptyEl);
        return
    }
    quests.forEach((quest) => {
        ...
        badgeWrap.className = 'text-xs';
        const badge = document.createElement('span');
        const status = String(quest.status || '');
        const isCompleted = status === 'completed';
        badge.className = 'px-2 py-0.5 rounded-full text-xs font-semibold ' + (isCompleted ? 'bg-green-700 text-green-200' : 'bg-blue-700 text-blue-100');
        ...
        const required = Number.isFinite(quest.required_amount) ? quest.required_amount : 0;
        const progress = Number.isFinite(quest.progress_amount) ? quest.progress_amount : 0;
        if (required > 0) {
            const prog = document.createElement('div');
            prog.className = 'text-xs text-gray-400 mb-2';
            prog.textContent = `Progress: ${progress} / ${required}`;
            card.appendChild(prog)
        }
        if (isCompleted && quest.quest_id) {
            const form = document.createElement('form');
            form.method = 'post';
            form.className = 'mt-2';
            const csrfInput = document.createElement('input');
            ...
            actionInput.type = 'hidden';
            actionInput.name = 'action';
            actionInput.value = 'turn_in_quest';
            const idInput = document.createElement('input');
            idInput.type = 'hidden';
            idInput.name = 'quest_id';
            idInput.value = String(quest.quest_id);
            const button = document.createElement('button');
            button.type = 'submit';
            button.className = 'px-3 py-1.5 rounded-md bg-purple-600 hover:bg-purple-500 text-xs font-semibold text-white';
