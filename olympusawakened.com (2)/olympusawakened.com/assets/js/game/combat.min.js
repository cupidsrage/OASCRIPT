...
(function initCombatModule() {
    ...
    const autoConfig =
        ...
        {};
    const autoAvailable = !!autoConfig.unlocked && (autoConfig.enabled === undefined || autoConfig.enabled !== false);
    ...
    const state = {
        ...
        currentMonsterId: parsedConfig.currentMonsterId !== undefined ?
        ...
        canAttack: true,
        actionDelayMs: initialActionDelayMs > 0 ? initialActionDelayMs : fallbackActionDelayMs,
        nextActionReadyAt: 0,
        delayActive: false,
    ...
    function syncDelayFromPayload(payload, sourceAction) {
        ...
        if (Number.isNaN(remaining) || remaining < 0) {
            ...
            if (readyAtMs > 0) {
                ...
                const now = Date.now();
                remaining = !Number.isNaN(serverNow) && serverNow > 0 ?
                    readyAtMs - serverNow :
                    readyAtMs - now;
            }
        }
    ...
    function appendPvpLogEvents(events) {
        ...
        const mapped = events.map((ev) => ({
            text: ev && ev.text !== undefined && ev.text !== null ?
                String(ev.text) :
                '',
            type: ev && ev.type !== undefined && ev.type !== null ?
                String(ev.type) :
                'neutral',
            target: ev && ev.target !== undefined && ev.target !== null ?
                String(ev.target) :
                '',
        }));
    ...
    async function sendAction(action, extra) {
        ...
        try {
            ...
            // Network or fetch error: re‑enable attacking and reset local cooldown so the UI isn't stuck.
            state.canAttack = true;
            cooldownLocalOnly = false;
        } finally {
    ...
    function mainAction() {
        ...
        if (isSwitching) {
            state.canAttack = false;
            sendAction('start_fight', {
                monster_id: selectedId
            });
            return;
        }
    ...
    function refightLast() {
        ...
        const extra = {
            monster_id: state.lastMonsterId
        };
        if (state.autoMode) {
            extra.auto_combat = '1';
        }
