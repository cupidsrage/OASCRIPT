...
(function initChat() {
    ...
    function fetchItemTooltip(linkId) {
        ...
        return fetch('api/chat_item_tooltip.php?id=' + encodeURIComponent(String(linkId)), {
    ...
    messagesEl.addEventListener('click', function handleNameClick(e) {
        ...
        const lineEl = findClosest(el, 'div', messagesEl);
        if (!lineEl || lineEl === messagesEl) {
            return;
        }
        const lineText = (lineEl.textContent || '');
        const normalizedLine = lineText.replace(/[\u00a0\u200b\ufeff]/g, ' ').trim();
        const match = /^[^A-Za-z]*(?:To|From)\s+([^:]+):/i.exec(normalizedLine);
        if (!match) {
            return;
        }
